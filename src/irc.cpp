...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("zcashdark-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #zcashdarkTEST2\r");
            Send(hSocket, "WHO #zcashdarkTEST2\r");
        } else {
            // randomly join #zcashdark00-#zcashdark05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #zcashdark%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #zcashdark%02d\r", channel_number).c_str());
        }
