...
bool CDBEnv::Open(boost::filesystem::path pathEnv_)
{
    ...
    if (!dbenv.get_lk_max_locks(&nMaxLocks))
    {
        ...
        {
            if (nBlocks < 1)
                strMessage = strprintf(_("Warning: DB_CONFIG has set_lk_max_locks %u, which may be too low for a single block. If this limit is reached, ZcashDARK may stop working."), nMaxLocks);
            else
                strMessage = strprintf(_("Warning: DB_CONFIG has set_lk_max_locks %u, which may be too low for a common blockchain reorganization. If this limit is reached, ZcashDARK may stop working."), nMaxLocks);
