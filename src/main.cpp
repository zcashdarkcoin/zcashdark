...
const int nModifierInterval = 6 * 60 * 60; // time to elapse before new modifier is computed

const int nCoinbaseMaturity = 30;
...
const string strMessageMagic = "ZcashDARK Signed Message:\n";
...
CBigNum inline GetProofOfStakeLimit(int nHeight, unsigned int nTime)
{
    ...
    if(true) // proof of stake on main-net
        return bnProofOfStakeLimit;
...
{
    const int nMinSubsidy = 1 * COIN;
    int halvings = nHeight / 50000;
    if (halvings >= 64) {
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("zcashdark-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in ZcashDARK
...
bool CBlock::AcceptBlock()
{
    ...
    if (fTestNet || pindexPrev->nTime < 1476305866)
        nMaxOffset = 7 * nOneWeek;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ZcashDARK", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "30.10.2016: Italy earthquake: 6.6-magnitude tremor rocks nation's center (CNN)";
        CTransaction txNew;
        txNew.nTime = 1477855212;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print        
        block.print();
        assert(block.hashMerkleRoot == uint256("0x1fe7adc2716cc8df9c4a02e92fdae3966ae323be27102e2b429f32144668fbdd"));
...
unsigned char pchMessageStart[4] = { 0xb4, 0xa6, 0xfa, 0x09 };
