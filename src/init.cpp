...
void Shutdown(void* parg)
{
    ...
    RenameThread("zcashdark-shutoff");
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("ZcashDARK version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  zcashdarkd [options]                     " + "\n" +
                  "  zcashdarkd [options] <command> [params]  " + _("Send command to -server or zcashdarkd") + "\n" +
                  "  zcashdarkd [options] help                " + _("List commands") + "\n" +
                  "  zcashdarkd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "zcashdark:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("ZcashDARK"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("ZcashDARK"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: zcashdark.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: zcashdarkd.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 7785 or testnet: 17785)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 12176 or testnet: 22176)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    SoftSetArg("-addnode", "45.63.10.53");
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  ZcashDARK is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "ZcashDARK server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("ZcashDARK"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of ZcashDARK") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart ZcashDARK to complete") << "\n";
