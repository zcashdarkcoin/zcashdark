ZcashDARK-qt: Qt4 GUI for ZcashDARK
===============================
...
Alternatively, install Qt Creator and open the `zcashdark-qt.pro` file.

An executable named `zcashdark-qt` will be built.
...
To use UPnP for port forwarding behind a NAT router (recommended, as more connections overall allow for a faster and more stable zcashdark experience), pass the following argument to qmake:
...
A warning for people using the *static binary* version of ZcashDARK on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of ZcashDARK is linked against libdb5.3.
...
installed causes zcashdark-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
